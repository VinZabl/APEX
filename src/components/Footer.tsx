...
const Footer: React.FC = () => {
  ...
  const footerColumns = [
    {
      title: siteSettings?.site_name || 'APEX Game Credits',
      links: [
  ...
  return (
    <footer className="mt-16" style={{ backgroundColor: '#0A0A0A', borderTop: '1px solid rgba(231, 70, 148, 0.2)' }}>
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 md:py-12">
        ...
        {/* Separator Line */}
        <div id="footer-separator" className="border-t border-cafe-primary/30 my-8"></div>
        ...
        <div className="flex flex-col items-center gap-6">
              ...
              {socialLinks.map((link, index) => {
                ...
                return (
                  <a
                    ...
                    className="w-10 h-10 rounded-full border-2 border-cafe-primary/50 flex items-center justify-center hover:bg-cafe-primary hover:border-cafe-primary transition-all duration-200 group"
                    aria-label={link.label}
                  >
                    <Icon className="h-5 w-5 text-cafe-primary group-hover:text-white transition-colors" />
                  </a>
                );
              })}
            </div>
          ...
          <div className="text-center">
            <p className="text-sm text-cafe-textMuted">
              © {new Date().getFullYear()} {siteSettings?.site_name || 'APEX Game Credits'}. All rights reserved.
            </p>
          </div>
        </div>
      </div>
